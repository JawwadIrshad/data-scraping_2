...
class PagePool:
    ...
    async def initialize(self):
        ...
        print(f"🚀 Initializing {self.size} browser tabs...")
        ...
        print(f"✅ Browser pool ready with {self.size} tabs")
...
async def scrape_single_url_optimized(url: str, page, idx: int, batch_number: int):
    ...
    while not scraped and retries > 0:
        try:
            ...
            scraped = True
            print(f"   ✅ [{batch_number}-{idx}] Success: {row_dict['business_name'][:30]} | {row_dict['phone']}")
            
        except PlaywrightTimeoutError:
            print(f"   ⏱️ [{batch_number}-{idx}] Timeout - retrying...")
            retries -= 1
            if retries > 0:
                await page.goto("about:blank")  # Clear page
                await asyncio.sleep(0.5)
        except Exception as e:
            print(f"   ⚠️ [{batch_number}-{idx}] Error: {str(e)[:50]}")
            retries -= 1
            if retries > 0:
                await page.goto("about:blank")  # Clear page
                await asyncio.sleep(0.5)
    ...
    if not scraped:
        print(f"   ❌ [{batch_number}-{idx}] Failed after {MAX_RETRIES} attempts")
...
async def process_batch_data_async(batch_data, all_sheets, batch_number):
    ...
    print(f"📊 Batch {batch_number}: {len(clean_data)} clean, {len(missing_data)} missing")
...
async def scrape_async_optimized():
    ...
    async with async_playwright() as p:
        ...
        while True:
            ...
            print(f"\n{'=' * 70}")
            print(f"🔄 BATCH {batch_number} - Processing {len(urls_batch)} URLs")
            print(f"📊 Remaining: {len(available_urls) - len(urls_batch)}")
            print(f"{'=' * 70}")
            ...
            print(f"✅ Processed: {scraped_count} URLs in {batch_time:.1f}s")
            print(f"⚡ Current speed: {current_speed:.1f} URLs/min")
            print(f"📊 Total: {total_processed} URLs")
            print(f"⏱️ Overall speed: {overall_speed:.1f} URLs/min")
    ...
    print(f"\n🎉 SCRAPING COMPLETED!")
    print(f"📊 Total URLs: {total_processed}")
    print(f"⏱️ Total time: {total_duration/60:.1f} minutes")
    print(f"⚡ Average speed: {total_processed/(total_duration/60):.1f} URLs/min")
